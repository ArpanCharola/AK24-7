...
const EMPTY_ANALYTICS = { kpis: {}, daily: [], sources: [], roles: [] };
...
export function AnalyticsOverview() {
  ...
  const cards = [
    ["Raw found today", kpis.raw_found_today, Activity], ["Accepted today", kpis.accepted_today, BriefcaseBusiness],
    ["Found yesterday", kpis.raw_found_yesterday, Layers3], ["Live jobs", kpis.live_jobs, Database],
    ["Duplicates today", kpis.duplicates_today, Layers3], ["Rejected today", kpis.rejected_today, ShieldAlert],
  ];
  return <section className="admin-view">
    <div className="admin-view-head"><div><p className="admin-eyebrow">WAREHOUSE PULSE</p><h2>Aggregation overview</h2><p>Posting activity, source contribution, and warehouse health in Asia/Kolkata.</p></div>
    ...
    <div className="admin-grid-2"><article className="admin-panel"><header><div><h3>Accepted jobs</h3><p>Unique roles admitted per day</p></div></header><SparkBars rows={data.daily || []}/></article>
      <article className="admin-panel"><header><div><h3>Demand coverage</h3><p>Profiles currently served</p></div></header><div className="admin-coverage"><strong>{number(kpis.profiles_with_matches)}</strong><span>profiles with matches</span><strong>{number(kpis.profiles_waiting)}</strong><span>waiting for coverage</span><strong>{number(kpis.active_clusters)}</strong><span>active demand clusters</span></div></article></div>
...
export function JobWarehouse() {
  const [filters, setFilters] = useState({ page: 1, page_size: 25, q: "", source: "", status: "live" });
  ...
  return <section className="admin-view"><div className="admin-view-head"><div><p className="admin-eyebrow">CANONICAL RECORDS</p><h2>All jobs</h2><p>Every deduplicated job with its source sightings and profile reach.</p></div><div className="admin-total">{number(data.total)}<span>jobs</span></div></div>
    <div className="admin-filters"><label><Search size={16}/><input value={filters.q} onChange={(e) => set("q", e.target.value)} placeholder="Title, employer, location…"/></label><select value={filters.source} onChange={(e) => set("source", e.target.value)}><option value="">All sources</option>{["linkedin","naukri","indeed","wellfound","glassdoor","cutshort","instahyre","hirist","ats"].map(x=><option key={x}>{x}</option>)}</select><select value={filters.status} onChange={(e) => set("status", e.target.value)}><option value="">All states</option><option value="live">Live</option><option value="quarantined">Quarantined</option><option value="expired">Expired</option><option value="closed">Closed</option></select></div>
